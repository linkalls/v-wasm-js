...
function resolveAppDir(appName) {
  ...
  const candidates = [
    ...
    path.join(cwd, 'benchmarks', appName),
  ];
...
function serveStatic(dir, port, opts = {}) {
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (opts.vitrio && pathname === '/vsignal.wasm') {
        const p = path.join(dir, 'dist', 'vsignal.wasm');
        if (fs.existsSync(p)) { res.writeHead(200, { 'Content-Type': 'application/wasm' }); fs.createReadStream(p).pipe(res); return; }
      }
      const tryPaths = [
      ...
      let found = null;
      for (const p of tryPaths) { if (fs.existsSync(p) && fs.statSync(p).isFile()) { found = p; break; } }
...
async function getFileSizeSafe(p) { try { const s = await stat(p); return s.size; } catch { return 0; } }
...
async function runBenchmark() {
  ...
  const results = { vitrio: { load: 0, interact: 0, listUpdate: 0, size: 0 }, solid: { load: 0, interact: 0, listUpdate: 0, size: 0 }, react: { load: 0, interact: 0, listUpdate: 0, size: 0 } };

  results.vitrio.size = await getFileSizeSafe(path.join(vitrioDir, 'dist', 'main.js'));
  try { const files = await readdir(path.join(solidDir, 'dist', 'assets')); const jsFile = files.find(f => f.endsWith('.js')); if (jsFile) results.solid.size = await getFileSizeSafe(path.join(solidDir, 'dist', 'assets', jsFile)); } catch (e) { console.error('Error checking solid assets', e); }
  try { const files = await readdir(path.join(reactDir, 'dist', 'assets')); const jsFile = files.find(f => f.endsWith('.js')); if (jsFile) results.react.size = await getFileSizeSafe(path.join(reactDir, 'dist', 'assets', jsFile)); } catch (e) { console.error('Error checking react assets', e); }
  ...
  async function benchApp(name, url, target) {
    ...
    await page.evaluate(() => {
      ...
      if (input && addBtn) {
        for (let i = 0; i < 50; i++) { input.value = `Todo item ${i}`; input.dispatchEvent(new Event('input', { bubbles: true })); addBtn.click(); }
    ...
    const endList = performance.now(); results[target].listUpdate = endList - startList; console.log(`  List update time: ${results[target].listUpdate.toFixed(2)}ms`);
    const listItems = await page.locator('li').count(); console.log(`  Final list items: ${listItems}`);
  }
  ...
  const listVitrioVsSolid = results.vitrio.listUpdate > 0 && results.solid.listUpdate > 0 ? ((results.solid.listUpdate / results.vitrio.listUpdate) * 100 - 100).toFixed(1) : 'N/A';
...
`;
  ...
  await browser.close(); s1.close(); s2.close(); s3.close(); process.exit(0);
}

runBenchmark().catch(err => { console.error('Benchmark error:', err); process.exit(1); });
